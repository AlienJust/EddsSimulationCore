...
namespace Bumiz.Apply.PulseCounterArchiveReader {
  ...
  public sealed class CountersSubSystem : CompositionPartBase, IPulseCounterDataStorageHolder {
    ...
    public CountersSubSystem() {
      ...
      _storage = new ConcurentPulseCounterDataStorage(
        new FilePulseCounterDataStorage(_counterInfos.Select(kvp => kvp.Value).ToList()));
    ...
    private void AsyncRecurseArchiveReadMethod(string objName, WaitableCounter sharedTasksCounter) {
      ...
      if (timeToGet.HasValue) {
        ...
        Log.Log("Есть архивы, которые нужно вычитать. Имя объекта=" + objName + "   Команда=<" + cmd.Comment +
                ">   Время=" + cmd.RequestedTime.ToSimpleString() + " Арх.№=" + cmd.RecordNumber);

        sharedTasksCounter.IncrementCount();
        _bumizIoManager.SendDataAsync(
          objName,
          cmd,
          result => {
            try {
              ...
              if (result != null) {
                if (result.ChannelException == null) {
                  Log.Log(result.Bytes.ToText() + " <= для времени = " + time.ToSimpleString() + " Арх.№=" +
                          cmd.RecordNumber);
                  try {
                    ...
                    if (ctResult.RecordTime.Date == time.Date) {
                      ...
                      _storage.SaveData(objName, time, true, ctResult.Count1, ctResult.Count2, ctResult.Count3,
                        ctResult.Status, ctResult.Xstatus);
                    }
                    else {
                    ...
                    Log.Log(
                      "Ошибка обработки ответа БУМИЗ, в хранилище будет записана информация о плохой записи архива");
                  ...
                  Log.Log("Объект " + objName +
                          " больше не будет опрашиваться в этой итерации (пока все остальные не закончат свои обмены)");
    ...
    public override void SetCompositionRoot(ICompositionRoot root) {
      ...
      foreach (var pulseCounterInfo in _counterInfos) {
        try {
          ...
          Log.Log("Не удалось связать информацию по импульсному счётчику " + pulseCounterInfo.Key +
                  " с информацией о его сетевом расположении внутри сети БУМИЗ по причине:" + ex);
        }
      }
