...
namespace Commands.Bumiz.CounterSe102 {
	public class SetTarProgCommand : ICounterCommand {
		...
		public SetTarProgCommand(TariffProgramType programType, int monthNumber, byte zeroBasedStartPointNumber, List<HalfAnHourAndTariffNumber> points) {
			_programType = programType;
			_monthNumber = monthNumber;
			if (zeroBasedStartPointNumber > 15) throw new Exception("Point number must be less than 16");
			if (points.Count < 1 || points.Count > 6) throw new Exception("Points count must be between 1 and 6 (including 1 and 6)");
