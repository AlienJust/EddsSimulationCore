...
namespace Commands.Bumiz.CounterSe102 {
	public class GetTarProgCommand : ICounterCommand {
		...
		public GetTarProgCommand(TariffProgramType programType, int monthNumber, byte zeroBasedStartPointNumber, byte pointsCount) {
			_programType = programType;
			_monthNumber = monthNumber;
			if (zeroBasedStartPointNumber > 15) throw new Exception("Point number must be less than 16");
			if (pointsCount < 1 || pointsCount > 6) throw new Exception("Points count must be between 1 and 6 (including 1 and 6)");
