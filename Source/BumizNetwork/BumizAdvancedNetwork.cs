...
namespace BumizNetwork {
  ...
  public class BumizAdvancedNetwork : IMonoChannel {
    ...
    private byte[] RequestSomethingCycle(ushort address, ushort? netAddress, byte[] buffer, int attemptsCount,
      int waitTimeout) {
      ...
      throw lastChannelException ??
            new Exception(
              "Не удалось осуществить обмен, однако и не удалось получить причину неудачи"); // Только что созданное исключение по идее не должно возникать, т.к. будет либо return, либо Timeout
    }
