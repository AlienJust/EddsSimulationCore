...
namespace PollServiceProxy {
  ...
  public sealed class PollGateway : CompositionPartBase, IPollGateway, ISubSystemRegistrationPoint {
    ...
    public PollGateway() {
      ...
      _microPacketSendingIntervalMs =
        XmlFactory.GetMicroPacketSendingIntervalMsFromXml(Path.Combine(Env.CfgPath, "PollServiceProxy.xml"));
      ...
      try {
        ...
        foreach (var scadaObjectInfo in _scadaObjects) {
          foreach (var objectScadaAddress in scadaObjectInfo.Value.ScadaAddresses) {
            if (objectScadaAddress.LinkName == scadaClient.Name &&
                objectScadaAddress.NetAddress == eventArgs.NetAddress) {
    ...
    public void SendData(string uplinkName, string scadaObjectName, byte commandCode, byte[] data) {
      ...
      _perScadaAddressWorkers[scadaAddress].AddWork(() =>
        SendReplyData(uplinkName, (ushort) scadaAddress.NetAddress, commandCode, data));
    }
