...
namespace Controllers.Gateway {
	...
	public class GatewayControllersSubSystem : CompositionPartBase, ISubSystem, IGatewayControllerInfosSystem {
		...
		public void ReceiveData(string uplinkName, string subObjectName, byte commandCode, byte[] data, Action notifyOperationComplete, Action<int, IEnumerable<byte>> sendReplyAction) {
			try {
				Log.Log("Получены данные для контроллера " + subObjectName + ": код команды=" + commandCode + ", байты данных: " + data.ToText());
					...
					try {
						...
						controller.GetDataInCallback(commandCode, data, (exception, bytes) => {
							try {
								...
								Log.Log("Данные были отправлены от лица контроллера " + subObjectName + ", байты данных: " + dataToSend.ToText());
							}
							catch (Exception ex) {
