...
namespace nMqtt {
	...
	public sealed class MqttClient : IDisposable {
		...
		public async Task<ConnectionState> ConnectAsync(string username = default(string), string password = default(string)) {
			...
			var msg = new ConnectMessage {ClientId = ClientId, CleanSession = CleanSession};
			...
			if (ConnectionState == ConnectionState.Connected) {
				_pingTimer = new Timer(state => { _conn.SendMessage(new PingReqMessage()); }, null, KeepAlive * 1000, KeepAlive * 1000);
			}
		...
		public void Publish(string topic, byte[] data, Qos qos = Qos.AtMostOnce) {
			Console.WriteLine(topic + " > need to publish data: " + data.ToText());
			
			var msg = new PublishMessage {FixedHeader = {Qos = qos}, MessageIdentifier = 0, TopicName = topic, Payload = data};
		...
		void DecodeMessage(byte[] buffer) {
			...
			switch (msg.FixedHeader.MessageType) {
				case MessageType.Connack:
					...
					if (connAckMsg.ConnectReturnCode == ConnectReturnCode.BrokerUnavailable || connAckMsg.ConnectReturnCode == ConnectReturnCode.IdentifierRejected || connAckMsg.ConnectReturnCode == ConnectReturnCode.UnacceptedProtocolVersion || connAckMsg.ConnectReturnCode == ConnectReturnCode.NotAuthorized || connAckMsg.ConnectReturnCode == ConnectReturnCode.BadUsernameOrPassword) {
						ConnectionState = ConnectionState.Disconnecting;
						Dispose();
						_conn = null;
						ConnectionState = ConnectionState.Disconnected;
				...
				case MessageType.Publish:
					...
					if (pubMsg.FixedHeader.Qos == Qos.AtLeastOnce) {
						var ackMsg = new PublishAckMessage {MessageIdentifier = pubMsg.MessageIdentifier};
