  ...
  public class TimeSyncSystem : CompositionPartBase {
            ...
            result => {
              try {
                if (result.ChannelException == null) {
                  ...
                  Log.Log("Время контроллера " + objectName + ": " + curControllerTime.ToString("yyyy.MM.dd-HH:mm:ss") +
                          (canSyncTime
                            ? " будет произведена синхронизация"
                            : " синхронизация не требуется или невозможна в автоматическом режиме"));
                }
                else {
    ...
    static bool CanSafeAndReallyNeedTimeSync(DateTime controllerTime, DateTime currentTime) {
      ...
      if (span.TotalMinutes > 2.0) {
        if (
          (lastTime.Minute > 2 && lastTime.Minute < 28 ||
           lastTime.Minute > 32 && lastTime.Minute < 58) &&
          (firstTime.Minute > 2 && firstTime.Minute < 28 ||
           firstTime.Minute > 32 && firstTime.Minute < 58)
        ) {
          return true;
        }
      }
      ...
      foreach (var objSyncInfo in _objectsToSync) {
        try {
          if (_bumizIoManager.BumizObjectExist(objSyncInfo)) {
            ...
            Log.Log("Не удалось связать информацию по объекту " + objSyncInfo +
                    " с информацией о его сетевом расположении внутри сети БУМИЗ, видимо конфигурация сетевого расположения отсутствует");
          }
        }
        catch (Exception ex) {
          Log.Log("Не удалось связать информацию по объекту " + objSyncInfo +
