...
namespace ScadaClient.Udp {
  public sealed class QueuedScadaClient : IScadaClient {
    ...
    public QueuedScadaClient(IPAddress serverIpAddress, int serverPort, int localPort) {
      ...
      _receiveWorker =
        new SingleThreadedRelayQueueWorkerProceedAllItemsBeforeStopNoLog<Action>("ScadaUdpClientReceiveWorker",
          a => a(), ThreadPriority.BelowNormal, true, null);
      _receiveWorker.AddWork(DoReceiveInBackThread);

      _proceedReceivedDataWorker =
        new SingleThreadedRelayQueueWorkerProceedAllItemsBeforeStopNoLog<Action>(
          "ScadaUdpClientProceedReceivedDataWorker", a => a(), ThreadPriority.Normal, true, null);
      _sendDataWorker =
        new SingleThreadedRelayQueueWorkerProceedAllItemsBeforeStopNoLog<Action>("ScadaUdpClientSendDataWorker",
    ...
    public void SendData(ushort netAddress, byte commandCode, byte[] data) {
      _sendDataWorker.AddWork(() => {
        ...
        _client.Send(buffer, buffer.Length,
          _serverEndPoint); // no timeout, no chance to know wether data were transmitted
      });
    }
    ...
    public void SendMicroPacket(ushort netAddress, byte linkLevel) {
      _sendDataWorker.AddWork(() => {
        ...
        buffer[3] = linkLevel;
        _client.Send(buffer, buffer.Length,
          _serverEndPoint); // no timeout, no chance to know wether data were transmitted, yeah its UDP ><
      });
    }
