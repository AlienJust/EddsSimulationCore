        ...
        public void SendMicroPacket(ushort netAddress, byte linkLevel)
        {
            _sendDataWorker.AddWork(() =>
            {
                if (netAddress > 255)
                {
                    ...
                    buffer[2] = netAddrB0;
                    buffer[3] = linkLevel;
                    _client.Send(buffer, buffer.Length, _serverEndPoint); // no timeout, no chance to know whether data were transmitted, yeah its UDP ><
                }
                else if (netAddress < 255)
                {
                    var netAddrB0 = (byte) (netAddress & 0xFF);
                ...
                // Else, if Intelecon network address equals 255 do nothing
                // (it is broadcast address and is not used by controllers)
            });
        }
