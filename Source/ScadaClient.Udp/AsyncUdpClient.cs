...
namespace ScadaClient.Udp {
  public class AsyncUdpClient : IScadaClient {
    ...
    static readonly ILogger Log = new RelayMultiLogger(true,
      ...
      new RelayLogger(new ColoredConsoleLogger(ConsoleColor.Magenta, Console.BackgroundColor),
        new ChainedFormatter(new ITextFormatter[]
          {new ThreadFormatter(" > ", false, true, false), new DateTimeFormatter(" > ")})));
    ...
    public AsyncUdpClient(string targetIp, int targetPort, int localPort, int connectionDropPeriodMs) {
      ...
      _notifyThreadWorker =
        new SingleThreadedRelayQueueWorkerProceedAllItemsBeforeStopNoLog<Action>("AsyncUDPClientBackThread", a => a(),
          ThreadPriority.Normal, true, null); // Нотификатор получения данных

      EstablishNewConnection();

      _dropConnectionWachdog = new Thread(WachForConnectionDropFunc) {
        IsBackground = true,
        Priority = ThreadPriority.BelowNormal,
        Name = "ConnectionDropWachdog"
      };
    ...
    void WachForConnectionDropFunc() {
      while (true) {
        ...
        Disconnect(_localUdpSocket,
          new Exception("Подключение сброшено преднамеренно (период сброса = " + _connectionDropTimeMs + "мс)"));
        EstablishNewConnection();
      }
    }
    ...
    public void EstablishNewConnection() {
      ...
      try {
        ...
        _localUdpSocket.BeginConnect(_targetEndpoint, ConnectCallback,
          _localUdpSocket); // Сокет передается, чтобы не заморачиваться с проверкой, тот ли это самый объект
    ...
    void ConnectCallback(IAsyncResult ar) {
      ...
      try {
        ...
        localSocket.BeginReceive(socketWithBuffer.B, 0, socketWithBuffer.B.Length, SocketFlags.None, ReceivedCallback,
          socketWithBuffer);
    ...
    void ReceivedCallback(IAsyncResult ar) {
      ...
      try {
        ...
        try {
          socketWithBuffer.S.BeginReceive(socketWithBuffer.B, 0, socketWithBuffer.B.Length, SocketFlags.None,
            ReceivedCallback, socketWithBuffer);
        }
        catch (Exception ex) {
            ...
            try {
              if (bytesToRead >= 8) {
                if (DataReceived != null) {
                  ...
                  Logg("Данные в формате Интелекон успешно получены, netAddr=" + netAddr + " cmdCode=" + cmdCode +
                       " info=" + rcvData.ToText() + ", оповещаем подписчиков");
                  DataReceived.SafeInvoke(this,
                    new DataReceivedEventArgs(netAddr, cmdCode, rcvData)); // Вызывается не в основном потоке!
                }
              }
            }
            catch (Exception ex) {
