...
namespace ScadaClient.Udp
{
    public class AsyncUdpClient : IScadaClient
    {
        ...
        public AsyncUdpClient(string targetIp, int targetPort, int localPort, int connectionDropPeriodMs)
        {
            ...
            _dropConnectionWachdog = new Thread(WatchForConnectionDropFunc) {IsBackground = true, Priority = ThreadPriority.BelowNormal, Name = "ConnectionDropWachdog"};
            _dropConnectionWachdog.Start();
        }

        void WatchForConnectionDropFunc()
        {
            while (true)
            {
        ...
        public void EstablishNewConnection()
        {
            ...
            try
            {
                ...
                localSocket.BeginConnect(_targetEndpoint, ConnectCallback, localSocket); // Сокет передается, чтобы не заморачиваться с проверкой, тот ли это самый объект
