...
namespace Audience {
	public static class Extensions {
		...
		public static byte[] GetNetBuffer(this byte[] buffer, ushort netAddress, byte commandCode) {
			...
			//                                  00    01                         02           03         04
			var resultBuffer = new List<byte> {0x7A, (byte) (buffer.Length + 4), commandCode, netAddrB1, netAddrB0};
		...
		public static void CheckInteleconNetBufCorrect(this byte[] netBuf, byte? cmdCode, ushort? inteleconAddr) {
			...
			if (cmdCode != null)
				if (netBuf[2] != cmdCode) throw new Exception("Wrong command code");

			if (inteleconAddr != null)
				if (netBuf[3]*0x100 + netBuf[4] != inteleconAddr) throw new Exception("Wrong net address");


			if (netBuf[netBuf.Length - 1 - 2] != netBuf.ToList().GetRange(1, netBuf.Length - 1 - 3).DallasCrc8()) throw new Exception("Wrong CRC");
			if (netBuf[netBuf.Length - 1 - 1] != netBuf.ToList().GetRange(1, netBuf.Length - 1 - 2).CtrlSum()) throw new Exception("Wrong CS");
		}
